...
class Game:
    def __init__(self):
        ...
        pg.display.set_caption(TITLE + " P1")
    ...
    def envia_dados(self):
        self.MESSAGE = str(self.player.pos.x)+ " " +str(self.player.pos.y)+ " "+\
            ...
            str(self.player.isidle)+" "+ str(self.player.pulando)+" "+ str(self.player.andando)+" "+\
            str(self.player.morto)
        # print(MESSAGE)
        ...
        try:
            ...
            if str(Pos[5]) == "True": self.player2.isidle = True
            else: self.player2.isidle = False
            if str(Pos[6]) == "True": self.player2.pulando = True
            else: self.player2.pulando = False
            if str(Pos[7]) == "True": self.player2.andando = True
            else: self.player2.andando = False
    ...
    def update(self):
        ...
        if hit_p2:
            if hit_p2[0].vel.x < self.player.vel.x:
                        self.player.pos.x = hit_p2[0].rect.left-(self.player.widht/2)
                        self.player.vel.x = hit_p2[0].vel.x
            if hit_p2[0].vel.x > self.player.vel.x:
                        self.player.pos.x = hit_p2[0].rect.right+(self.player.widht/2)
                        self.player.vel.x = hit_p2[0].vel.x
        ...
        if bullet_coll:
            if bullet_coll[0].cor != 0:
                if bullet_coll[0].speed < 0:
                    ...
                    self.player.vel.x = -(self.player.widht/2)
                if bullet_coll[0].speed > 0:
    ...
    def mostra_vencedor(self):
        ...
        else:self.text = self.font.render('Você perdeu, o jogador 2 venceu', True, BLACK)
